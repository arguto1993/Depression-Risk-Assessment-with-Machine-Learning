...
def create_age_distribution_chart(df):
    ...
    df_copy["Age_Group"] = pd.cut(
        df_copy["Age"], bins=age_bins, labels=age_labels, include_lowest=True
    )

    grouped_data = (
        df_copy.groupby(["Age_Group", "Depression"]).size().reset_index(name="Count")
    )
    grouped_data["Depression"] = grouped_data["Depression"].map({1: "Yes", 0: "No"})
...
def create_map_visualization(df):
    ...
    city_depression = (
        city_data.groupby("City")["Depression"].agg(["sum", "count"]).reset_index()
    )
    ...
    city_depression["lat"] = city_depression["City"].map(
        lambda x: city_coords.get(x, (0, 0))[0]
    )
    city_depression["lon"] = city_depression["City"].map(
...
def render_dashboard_page():
    ...
    with col5:
        st.plotly_chart(
            create_horizontal_bar_chart(filtered_df, "Gender", "Depression by Gender"),
            use_container_width=True,
        )
    ...
    with col7:
        ...
        if len(filtered_df) > 0:
            top_professions = (
                filtered_df["Profession"].value_counts().head(10).index.tolist()
            )
        ...
        st.plotly_chart(
            create_horizontal_bar_chart(
                df_prof, "Profession", "Depression by Profession"
            ),
            use_container_width=True,
        )
    ...
    with col8:
        st.plotly_chart(
            create_horizontal_bar_chart(
                filtered_df, "Have you ever had suicidal thoughts ?", "Thought Suicide?"
            ),
            use_container_width=True,
        )
    ...
    with col9:
        ...
        st.plotly_chart(
            create_grouped_bar_chart(
                df_degree, "Degree", "Degree Distribution", "Degree"
            ),
            use_container_width=True,
        )
    ...
    st.plotly_chart(
        create_age_distribution_chart(filtered_df), use_container_width=True
    )
