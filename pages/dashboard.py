...
def render_dashboard_page():
    ...
    with col3:
        st.plotly_chart(create_grouped_bar_chart(df, 'Academic Pressure', 
    ...
    with col4:
        st.plotly_chart(create_map_visualization(df), use_container_width=True)
    
    with col5:
        st.plotly_chart(create_horizontal_bar_chart(df, 'Gender', 
                                                     'Depression by Gender'), 
                        use_container_width=True)
    
    with col6:
        st.plotly_chart(create_grouped_bar_chart(df, 'Financial Stress', 
    ...
    with col7:
        # Get top professions
        top_professions = df['Profession'].value_counts().head(10).index.tolist()
        df_prof = df[df['Profession'].isin(top_professions)]
        st.plotly_chart(create_horizontal_bar_chart(df_prof, 'Profession', 
                                                     'Depression by Profession'), 
                        use_container_width=True)
    
    with col8:
        st.plotly_chart(create_horizontal_bar_chart(df, 'Have you ever had suicidal thoughts ?', 
                                                     'Thought Suicide?'), 
                        use_container_width=True)
    
    with col9:
        # Get top degrees
        top_degrees = df['Degree'].value_counts().head(10).index.tolist()
        df_degree = df[df['Degree'].isin(top_degrees)]
